...
# import/ export data by right clicking at table names
...
SELECT DISTINCT first_name, last_name, salary, hire_date
FROM employees
ORDER BY 3,4;

SELECT first_name, last_name, salary, salary*1.2 as bonus
FROM employees  #doesnt chnage the original table, to change need to use ADD COLUMN function
ORDER BY salary DESC;

# base relation is the tables, view relation is from query
...
WHERE job_id=9 AND YEAR(hire_date)>1995;

SELECT first_name, last_name, salary FROM employees
WHERE salary BETWEEN 7000 AND 10000;

#begin with A #take last name
SELECT last_name FROM employees 
WHERE last_name LIKE 'A%'; #for LIKE function must have string, VAR/VARCHAR
...
#2nd character is a and any between character
SELECT last_name FROM employees 
WHERE last_name LIKE '_a%e%';

SELECT department_id, COUNT(employee_id) AS staff_count # COUNT is an example of aggregate
FROM employees
